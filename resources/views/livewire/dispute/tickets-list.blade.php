<div class="flex flex-col h-full bg-gray-100 shadow-md">
    <button class="absolute top-3 right-3 lg:hidden" id="close-sidebar"><i class="fas fa-times"></i></button>
    @if($tickets)
        @foreach($tickets as $ticket)
            <div class="{{ $selectedTicket == $ticket->id ? 'bg-[#2545C3] text-white' : ''}} flex items-center p-5 cursor-pointer transition-all duration-200 my-1 rounded-[20px] relative group hover:bg-gradient-to-t hover:from-[#2545C3] hover:to-[#3959D5] hover:text-white text:black">
